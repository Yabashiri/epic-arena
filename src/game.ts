namespace Arena {
    ...
    export class Game extends Phaser.Game {
        constructor(elementName: string) {
            ...
            super(Global.Constants.gameWidth, Global.Constants.gameHeight, Phaser.AUTO, element.id, Arena.State.Blank);
        ...
        public connect(connectionDetails: IConnectionDetails, callback: (socket: SocketIOClient.Socket) => {}): void {
            Global.socket = io.connect(Global.Constants.serverUrl, { query: `name=${connectionDetails.name}&title=${connectionDetails.title}` });
